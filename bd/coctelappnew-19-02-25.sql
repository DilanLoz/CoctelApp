...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 19-02-2025 a las 16:25:41
...
(2002, 'Cabañas', 'Juan Martinez', 8029812, 'caba@gmail.com', 300940912, 'e0851d399bb2954f554d93328662c70f79273f5a', 'Calle 20 #40-60 Norte', '09:00 - 17:00', NULL, 1, 30, 102),
(2003, 'Bazar', 'Jose Perez', 8039923, 'baz@gmail.com', 300828821, 'a7c9f5b4f15002ac1275c418d5c77f316ae749db', 'Transversal 7 #30-50 ', '09:00 - 19:00', NULL, 2, 30, 102),
...
(40010, 'cabaaa', '', 2374772, 'adad@jhad.com', 31312, 'qffas', '3dadw', '11', NULL, 2, 30, 104),
(60001, 'cab3', '', 1212121, 'ca3@gm.com', 11212122, 'wwww', '11-22', '12', NULL, 4, 30, 104);
...
CREATE TRIGGER `after_insert_bar` AFTER INSERT ON `bar` FOR EACH ROW BEGIN
    -- Verificar si la inserción es para un bar (idper = 30)
    IF NEW.idper = 30 THEN
        -- Insertar los datos del bar en la tabla usuario
        INSERT INTO usuario (
            idusu, nomusu, emausu, celusu, numdocu, fotiden, pssusu, codubi, idval, idper, nompropi, dircbar, horbar
        ) VALUES (
            NEW.idbar, NEW.nombar, NEW.emabar, NEW.telbar, NEW.nit, NEW.fotbar, NEW.pssbar, NEW.codubi, NEW.idval, NEW.idper, NEW.nompropi, NEW.dircbar, NEW.horbar
        );
    END IF;
END
$$
...
CREATE TRIGGER `after_update_bar` AFTER UPDATE ON `bar` FOR EACH ROW BEGIN
    -- Si el idper corresponde a bar (30), actualizar en la tabla usuario
    IF NEW.idper = 30 THEN
        UPDATE usuario
        SET 
            nomusu = NEW.nombar,
...
CREATE TABLE `carrito` (
  ...
  `metodo_pago` varchar(20) DEFAULT 'Contraentrega'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
INSERT INTO `carrito` (`idcarrito`, `idusu`, `fecha_creacion`, `estado`, `metodo_pago`) VALUES
(2, 1, '2025-02-08', 'activo', 'Contraentrega'),
(3, 1, '2025-02-08', 'activo', 'Contraentrega'),
(4, 1, '2025-02-08', 'activo', 'Contraentrega'),
(5, 1, '2025-02-08', 'activo', 'Contraentrega'),
(6, 1, '2025-02-08', 'activo', 'Contraentrega'),
(7, 1, '2025-02-08', 'activo', 'Contraentrega'),
(8, 1, '2025-02-08', 'activo', 'Contraentrega'),
(10, 1, '2025-02-08', 'activo', 'Contraentrega');
...
INSERT INTO `detcarrito` (`iddetcarrito`, `idcarrito`, `idprod`, `cantidad`, `precio_unitario`) VALUES
(4, 2, 21, 2, 24600.00),
(5, 2, 23, 1, 229900.00),
(6, 2, 25, 3, 72000.00),
(7, 3, 21, 2, 24600.00),
...
CREATE TRIGGER `before_insert_detcarrito` BEFORE INSERT ON `detcarrito` FOR EACH ROW BEGIN
    ...
    -- Verificar si el idcarrito existe y está activo
    SELECT COUNT(*) INTO v_existe
    FROM carrito
    WHERE idcarrito = NEW.idcarrito AND estado = 'activo';

    -- Si el carrito no existe o no está activo, evitar el insert
    IF v_existe = 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'El carrito especificado no existe o no está activo.';
    END IF;
END
$$
DELIMITER ;
...
INSERT INTO `detpedido` (`iddetpedido`, `idpedido`, `idprod`, `cantidad`, `precio_unitario`, `idusu`) VALUES
(109301, 100110, 36, 20, 200000.00, 2003);
...
CREATE TRIGGER `trg_detpedido_after_delete` AFTER DELETE ON `detpedido` FOR EACH ROW BEGIN
    UPDATE pedido 
    SET total = (SELECT COALESCE(SUM(subtotal), 0) FROM detpedido WHERE idpedido = OLD.idpedido)
...
CREATE TRIGGER `trg_detpedido_after_insert` AFTER INSERT ON `detpedido` FOR EACH ROW BEGIN
    UPDATE pedido 
    SET total = (SELECT SUM(subtotal) FROM detpedido WHERE idpedido = NEW.idpedido)
...
(2010, 'juan', 'juan@kasd.com', 3213, '2025-02-11', 12313, NULL, 'adad', 2, 4008, 5, 20, 101),
(30001, 'Juann', 'adwd@ad.com', 213123, '2025-02-05', 1235423342, NULL, 'dasda', 2, 4008, 3, 20, 103);
...
CREATE TRIGGER `after_insert_empleado` AFTER INSERT ON `empleado` FOR EACH ROW BEGIN
    -- Verificar si la inserción es para un empleado (idper = 20)
    IF NEW.idper = 20 THEN
        -- Insertar los datos del empleado en la tabla usuario
        INSERT INTO usuario (
            idusu, nomusu, emausu, celusu, numdocu, fotiden, pssusu, codubi, idval, idper, idserv, idbar
        ) VALUES (
            NEW.idemp, NEW.nomemp, NEW.emaemp, NEW.celemp, NEW.numdocu, NEW.fotiden, NEW.pssemp, NEW.codubi, NEW.idval, NEW.idper, NEW.idserv, NEW.idbar
        );
    END IF;
END
$$
...
CREATE TRIGGER `after_update_empleado` AFTER UPDATE ON `empleado` FOR EACH ROW BEGIN
    -- Si el idper corresponde a empleado (20), actualizar en la tabla usuario
    IF NEW.idper = 20 THEN
        UPDATE usuario
        SET 
            nomusu = NEW.nomemp,
            ...
            idper = NEW.idper,
            idval = NEW.idval,
            fecnausu = NEW.fecnaemp  -- Agregado el campo fecnaemp
...
CREATE TABLE `pedido` (
  ...
  `idmetpago` tinyint(2) DEFAULT NULL,
...
INSERT INTO `pedido` (`idpedido`, `idcarrito`, `idmetpago`, `fecha_pedido`, `estado`, `total`, `idusu`, `idfactura`, `estado_pago`, `metodo_pago`) VALUES
(100110, 10, 4, '2024-12-02', 'Pendiente', 200000.00, 2001, NULL, 'pendiente', 'efectivo');
...
CREATE TRIGGER `trg_generar_factura` AFTER UPDATE ON `pedido` FOR EACH ROW BEGIN
    IF NEW.estado_pago = 'pagado' AND OLD.estado_pago = 'pendiente' THEN
        ...
        VALUES (NEW.idpedido, NOW(), NULL, NEW.total, NEW.idusu, 'emitida');
        ...
        INSERT INTO detfact (idfact, idprod, fecha, cantidad, precio_unitario, subtotal, idusu)
        SELECT 
            @idfactura, 
            dp.idprod, 
            NOW(), 
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trg_transferir_carrito_a_detpedido` AFTER INSERT ON `pedido` FOR EACH ROW BEGIN
    INSERT INTO detpedido (idpedido, idprod, cantidad, precio_unitario, subtotal, idusu)
    SELECT 
        NEW.idpedido, 
        dc.idprod, 
        dc.cantidad, 
        p.vlrprod,  
        dc.cantidad * p.vlrprod, 
        c.idusu
    FROM detcarrito dc
    JOIN carrito c ON dc.idcarrito = c.idcarrito
    JOIN producto p ON dc.idprod = p.idprod
    WHERE c.idcarrito = NEW.idcarrito;

    DELETE FROM detcarrito WHERE idcarrito = NEW.idcarrito;
END
$$
DELIMITER ;
...
(35, 'Black Russian 250ml', 'Coctel', 28000.00, '07276a65314afcbaac014573c8b9d4f4.png', NULL, 2002, 50, NULL, NULL, 'coctel'),
(36, 'Bramble 200ml', 'coctel', 18000.00, '15a38352e93759bc0ceaf1105f412cb9.png', NULL, 2003, 50, NULL, NULL, 'coctel');
...
(1, 'Dilan Lopez', 'dilan@gmail.com', NULL, NULL, 1053, '2004-09-11', 'e5c9b13cea0a3a0e4b38f906906390d85463388a', NULL, 10, 101, NULL, NULL, NULL, NULL, NULL, 1),
(4, 'Sebastian Martinez', 'sebas@gmail.com', NULL, NULL, 3030, NULL, '52109b25d2fba7f005f3052cad7c92a1861a48e1', NULL, 40, 101, NULL, NULL, NULL, NULL, NULL, 1),
...
(2001, 'Jose', 'jose@gmail.com', 3001782, NULL, 102891, '2000-11-09', '15bb060fd110a4f67c6b5b287072524f345ec1b7', 2, 20, 101, 2, 2003, 'null', NULL, NULL, 1),
(2002, 'Cabañas', 'caba@gmail.com', 300940912, NULL, 8029812, NULL, 'e0851d399bb2954f554d93328662c70f79273f5a', 1, 30, 102, NULL, NULL, 'Juan Martinez', 'Calle 20 #40-60 Norte', NULL, 1),
(2003, 'Bazar', 'baz@gmail.com', 300828821, NULL, 8039923, NULL, 'a7c9f5b4f15002ac1275c418d5c77f316ae749db', 2, 30, 102, NULL, NULL, 'Jose Perez', 'Transversal 7 #30-50 ', NULL, 1),
...
(4008, 'Caba2', 'caba@gamdo.ocm', 2341213, NULL, 11212, NULL, 'adsad', 1, 30, 104, NULL, NULL, 'Juan', 'adauav', '11pm', 1),
(30001, 'Juann', 'adwd@ad.com', 213123, NULL, 1235423342, NULL, 'dasda', 3, 20, 103, 2, 4008, NULL, NULL, NULL, 1),
(40010, 'cabaaa', 'adad@jhad.com', 31312, NULL, 0, NULL, 'qffas', 2, 30, 104, NULL, NULL, '', '3dadw', '11', 1),
(60001, 'cab3', 'ca3@gm.com', 11212122, NULL, 1212121, NULL, 'wwww', 4, 30, 104, NULL, NULL, '', '11-22', '12', 1);
...
--
ALTER TABLE `carrito`
  MODIFY `idcarrito` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=11;

--
-- AUTO_INCREMENT de la tabla `detcarrito`
--
ALTER TABLE `detcarrito`
  MODIFY `iddetcarrito` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=28;
...
--
ALTER TABLE `detpedido`
  MODIFY `iddetpedido` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=109305;
...
--
ALTER TABLE `pedido`
  MODIFY `idpedido` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=100118;
