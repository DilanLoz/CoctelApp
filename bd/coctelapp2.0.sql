...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 13-11-2024 a las 23:19:38
...
CREATE TRIGGER `after_insert_bar` AFTER INSERT ON `bar` FOR EACH ROW BEGIN
    -- Comprobar si el idbar ya existe como idusu
    DECLARE new_idusu INT;

    -- Generar un nuevo idusu único
    SELECT MAX(idusu) + 1 INTO new_idusu FROM usuario;
    ...
    VALUES (new_idusu, NEW.nombar, NEW.emabar, NEW.nit, NEW.nit, NULL, NEW.pssbar, NEW.codubi, NEW.idval, 30);
END
$$
DELIMITER ;
DELIMITER $$
...
CREATE TRIGGER `after_insert_empleado` AFTER INSERT ON `empleado` FOR EACH ROW BEGIN
    -- Comprobar si el idemp ya existe como idusu
    DECLARE new_idusu INT;

    -- Generar un nuevo idusu único para el usuario
    SELECT MAX(idusu) + 1 INTO new_idusu FROM usuario;

    -- Insertar en la tabla usuario cuando se inserte un nuevo empleado
    INSERT INTO usuario (idusu, nomusu, emausu, numdocu, celusu, fecnausu, pssusu, codubi, idval, idper)
    VALUES (new_idusu, NEW.nomemp, NEW.emaemp, NEW.numdocu, NEW.celemp, NEW.fecnaemp, NEW.pssemp, NEW.codubi, NEW.idval, 20);
END
$$
DELIMITER ;
DELIMITER $$
...
--
ALTER TABLE `producto`
  MODIFY `idprod` bigint(20) NOT NULL AUTO_INCREMENT COMMENT 'id de producto', AUTO_INCREMENT=9;
