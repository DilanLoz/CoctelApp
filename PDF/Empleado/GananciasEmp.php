...
$facturas = $facturaModel->getFacturasActivasPagadasMesActual($idusu);
...
class PDF extends TCPDF {
    ...
    public function Footer() {
        ...
        $this->Cell(0, 10, 'Fecha de impresión: ' . $this->fecha_impresion, 0, 0, 'C');
    }
}

$pdf = new PDF();
$pdf->fecha_impresion = $fecha_generacion;
...
$pdf->Cell(50, 10, 'Total (2%)', 1, 1, 'C', true);
...
foreach ($facturas as $factura) {
    $totalCalculado = $factura['total'] * 0.02; // 2% del total
    $pdf->Cell(40, 10, $factura['idfact'], 1, 0, 'C');
    $pdf->Cell(50, 10, $factura['fecha'], 1, 0, 'C');
    $pdf->Cell(50, 10, $factura['metodo_pago'], 1, 0, 'C');
    $pdf->Cell(50, 10, "$".number_format($totalCalculado, 2, ',', '.'), 1, 1, 'C');
    $totalGeneral += $totalCalculado;
}

// Total general (suma del 2% de todas las facturas)
$pdf->SetFont('dejavusans', 'B', 12);
$pdf->SetFillColor(80, 80, 80);
$pdf->SetTextColor(255, 255, 255);
$pdf->Cell(140, 10, 'Total General (2%)', 1, 0, 'R', true);
